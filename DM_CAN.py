...
class SocketCANMotorControl:
    """
    MotorControl class for SocketCAN interface (native CAN bus)
    用于SocketCAN接口的电机控制类（原生CAN总线）
    """
    #                4310           4310_48        4340           4340_48
    Limit_Param = [[12.5, 30, 10], [12.5, 50, 10], [12.5, 8, 28], [12.5, 10, 28],
                   # 6006           8006           8009            10010L         10010
    ...
    def __init__(self, can_interface='can0', bitrate=None):
        """
        Initialize SocketCAN motor control 初始化SocketCAN电机控制
        :param can_interface: CAN interface name (e.g., 'can0', 'can1') CAN接口名称
        :param bitrate: CAN bitrate (if None, assumes interface is already configured) CAN波特率
        """
        if not CAN_AVAILABLE:
            raise ImportError("python-can library not available. Install with: pip install python-can")
        ...
        try:
            ...
            self.bus = can.interface.Bus(channel=can_interface, bustype='socketcan', fd=True)

            print(f"SocketCAN interface {can_interface} opened successfully")
    ...
    def recv(self):
        """
        Receive CAN messages 接收CAN消息
        """
        # Set a short timeout to receive messages
    ...
    def recv_set_param_data(self):
        """
        Receive parameter setting responses 接收参数设置响应
        """
    ...
    def __process_can_message(self, msg):
        """
        Process received CAN message 处理接收到的CAN消息
        """
        if len(msg.data) < 6:
            return
        ...
        # Check if it's a motor feedback message
        if CANID != 0x00:
            if CANID in self.motors_map:
                q_uint = np.uint16((np.uint16(data[1]) << 8) | data[2])
                dq_uint = np.uint16((np.uint16(data[3]) << 4) | (data[4] >> 4))
    ...
    def __process_set_param_message(self, msg):
        """
        Process parameter setting response CAN message 处理参数设置响应CAN消息
        """
        if len(msg.data) < 8:
            return
    ...
    def save_motor_param(self, Motor):
        """
        save all parameters to flash 保存所有电机参数
        """
        can_id_l = Motor.SlaveID & 0xff
        can_id_h = (Motor.SlaveID >> 8) & 0xff
        data_buf = np.array([np.uint8(can_id_l), np.uint8(can_id_h), 0xAA, 0x00, 0x00, 0x00, 0x00, 0x00], np.uint8)
